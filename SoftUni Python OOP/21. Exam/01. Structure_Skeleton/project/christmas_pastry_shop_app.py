...
class ChristmasPastryShopApp:
    ...
    def add_booth(self, type_booth: str, booth_number: int, capacity: int):
        ...
        if type_booth == "Open Booth":
            self.booths.append(
                OpenBooth(booth_number=booth_number, capacity=capacity))
        elif type_booth == "Private Booth":
            self.booths.append(
                PrivateBooth(booth_number=booth_number, capacity=capacity))
    ...
    def leave_booth(self, booth_number: int):
        for booth in self.booths:
            if booth.booth_number == booth_number:
                boothToLeave = booth
                break
        priceToPay = boothToLeave.price_for_reservation + \
            sum([deli.price for deli in boothToLeave.delicacy_orders])
        boothToLeave.price_for_reservation = 0
        boothToLeave.delicacy_orders = []
        boothToLeave.is_reserved = False
